...
function fromFormData(fd: FormData): MonthlySummaryCreate {
  return {
    year: Number(fd.year),
    month: Number(fd.month),
    income: parseNum(fd.income),
    expense: parseNum(fd.expense),
    net_income: parseNum(fd.net_income),
    cumulative_net_income: parseNum(fd.cumulative_net_income),
    investment_principal: parseNum(fd.investment_principal),
    investment_value: parseNum(fd.investment_value),
  };
}
...
interface SummaryFormProps {
  initial: FormData;
  onSubmit: (data: MonthlySummaryCreate) => void;
  onCancel: () => void;
  isLoading: boolean;
  submitLabel: string;
}

function SummaryForm({ initial, onSubmit, onCancel, isLoading, submitLabel }: SummaryFormProps) {
  ...
  const numberField = (label: string, field: keyof FormData, required = false) => (
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 16px' }}>
        <div>{numberField('연도', 'year', true)}</div>
        <div>{numberField('월', 'month', true)}</div>
        <div>{numberField('수입 (원)', 'income')}</div>
        <div>{numberField('지출 (원)', 'expense')}</div>
        <div>{numberField('순수익 (원)', 'net_income')}</div>
        <div>{numberField('누적순수익 (원)', 'cumulative_net_income')}</div>
        <div>{numberField('투자원금 (원)', 'investment_principal')}</div>
        <div>{numberField('평가금 (원)', 'investment_value')}</div>
      </div>
...
function MonthlySummaryStatus() {
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, data }: { id: number; data: MonthlySummaryUpdate }) =>
      updateMonthlySummary(id, data),
  ...
  const handleEdit = (item: MonthlySummary) => {
    setEditTarget(item);
    setModalMode('edit');
  };
  ...
  return (
    <div style={cardStyle}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--md-space-md)' }}>
        <h2 style={{ ...titleStyle, marginBottom: 0 }}>월별 결산</h2>
      ...
      {!isLoading && !error && summaries.length === 0 && (
        <p style={{ font: 'var(--md-body-medium)', color: 'var(--md-sys-light-on-surface-variant)' }}>
          등록된 월별 결산 데이터가 없습니다. 추가 버튼을 눌러 직접 입력하거나 Excel 파일을 업로드하세요.
        </p>
      )}
      ...
      {!isLoading && !error && summaries.length > 0 && (
        <>
          ...
          <div style={{ marginTop: 'var(--md-space-md)', paddingTop: 'var(--md-space-md)', borderTop: '1px solid var(--md-sys-light-outline-variant)', overflowX: 'auto' }}>
              ...
              <tbody>
                {sorted.map((s) => (
                  <tr key={s.id}>
                    ...
                    <td style={{ ...tdStyle, textAlign: 'right', color: s.net_income != null && s.net_income >= 0 ? 'var(--md-sys-light-on-surface)' : 'rgba(186,26,26,0.9)' }}>
