      ...
      <div style={{ display: 'flex', gap: 'var(--md-space-sm)', justifyContent: 'flex-end', marginTop: 'var(--md-space-sm)' }}>
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 'var(--md-radius-sm)',
            border: 'none',
            background: 'var(--md-sys-light-primary)',
...
export default function CustomerInfo() {
  ...
  const editMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: CustomerCreate }) => updateCustomer(id, data),
    ...
    <div style={cardStyle}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 'var(--md-space-md)' }}>
        <h2 style={{ font: 'var(--md-title-small)', color: 'var(--md-sys-light-on-surface)', margin: 0 }}>
          고객정보 <span style={{ font: 'var(--md-label-small)', color: 'var(--md-sys-light-on-surface-variant)' }}>총 {customers.length}명</span>
        </h2>
        <button
      ...
      {customers.length === 0 ? (
        <p style={{ font: 'var(--md-body-medium)', color: 'var(--md-sys-light-on-surface-variant)' }}>
          등록된 고객정보가 없습니다. 추가 버튼을 눌러 입력하세요.
        </p>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--md-space-md)' }}>
          {customers.map((customer) => (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                ...
                <div style={{ display: 'flex', gap: 'var(--md-space-sm)', marginLeft: 'var(--md-space-md)' }}>
                  <button
                    onClick={() => setEditing(customer)}
                    style={{ padding: '4px 12px', borderRadius: 'var(--md-radius-sm)', border: '1px solid var(--md-sys-light-outline-variant)', background: 'transparent', cursor: 'pointer', font: 'var(--md-label-small)', color: 'var(--md-sys-light-on-surface-variant)' }}
                  >
                    편집
                  </button>
                  <button
                    onClick={() => window.confirm('삭제하시겠습니까?') && deleteMutation.mutate(customer.id)}
                    style={{ padding: '4px 12px', borderRadius: 'var(--md-radius-sm)', border: '1px solid rgba(186,26,26,0.4)', background: 'transparent', cursor: 'pointer', font: 'var(--md-label-small)', color: 'rgba(186,26,26,0.9)' }}
                  >
                    삭제
                  </button>
                </div>
      ...
      {editing && (
        <Modal title="고객 편집" onClose={() => setEditing(null)}>
          <CustomerForm
            initial={editing}
